...
class LearnModel(object):
    def __init__(self, mode, args=None, model_dir=None):
        if mode == 'train':
            self.args = args
            # self.data = Data(args)
            self.train_data = pickle.load(open('/home/yutongyan/PycharmProjects/rl-learn/learn/train_data_'+self.args.lang_enc+'.pkl', "rb"),  encoding='bytes')
            self.valid_data = pickle.load(open('/home/yutongyan/PycharmProjects/rl-learn/learn/valid_data_'+self.args.lang_enc+'.pkl', "rb"),  encoding='bytes')
            self.build_graph()
    ...
    def mlp(self, input_enc, output_shape, scope, layer_size, dropout=1., n_layers=3):
        with tf.variable_scope(scope):
            input_size = input_enc.get_shape().as_list()[-1]
            ws = []
            bs = []
            in_dim = [input_size] + [layer_size] * (n_layers - 1)
            out_dim = [layer_size] * (n_layers - 1) + [output_shape]
    ...
    def build_graph(self):
        ...
        self.action_enc = self.mlp(self.action, self.args.action_enc_size, "action_encoder", self.args.classifier_size)

        self.text_enc = self.compute_text_embedding()

        self.state = tf.placeholder(tf.float32, [None, self.args.state_latent_size * 5])
        self.state_enc = self.mlp(self.state, self.args.state_enc_size, 'state_encoder', 512)

        self.labels = tf.placeholder(tf.int32, [None], name='labels')

        self.action_text = tf.concat([self.action_enc, self.text_enc, self.state_enc], 1)
        self.logits = self.mlp(self.action_text, 2, "classifier", self.args.classifier_size, dropout=self.dropout)
    ...
    def run_batch_train(self, data, start):
        ...
        action_list, lang_list, label_list, state_list = zip(*curr_batch_data)
        ...
        batch_dict =    {    
                            ...
                            self.is_train: 1,
                            self.state: np.squeeze(state_list)
                        }
    ...
    def train_network(self):
        steps_per_epoch = int(math.ceil(len(self.train_data) / self.args.batch_size))
        n_epochs = 50
        ...
        with tf.Session() as self.sess:
            ...
            for epoch in range(epoch_start, n_epochs):
                ...
                acc_train, loss_train = self.run_epoch(self.train_data, is_train=1)
                acc_valid, loss_valid = self.run_epoch(self.valid_data, is_train=0)
    ...
    def predict(self, action_list, lang_list, state_list):
