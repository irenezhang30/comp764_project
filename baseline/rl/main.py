...
def main(args):
    ...
    # num_updates = math.ceil(10000 / (num_processes * num_steps))
    n_goal_reached = 0
    n_episodes = 0
    ...
    torch.save(actor_critic.state_dict(), '%d_%d_%f-seed%d.pt'%(args.expt_id, args.descr_id, args.lang_coeff,args.seed))

    print ("begin rollout")
    # DO NOT UPDATE
    with torch.no_grad():
        num_updates = math.ceil(10400 / (num_processes * num_steps))
        for j in range(num_updates):
            for step in range(num_steps):
                value, action, action_log_prob, recurrent_hidden_states = actor_critic.act(
                    rollouts.obs[step],
            ...
            if j % 10  == 0 or j == num_updates-1:
                ...
                try:
                    ...
                    success = 0.
                print("rollout: Timesteps: {}, Goal reached : {} / {}, Success %: {}".format(
                    total_num_steps, n_goal_reached, n_episodes, success))
