...
class LearnModel(object):
    ...
    # LSTM
    # def mlp(self, input_enc, output_shape, scope, n_hidden=256, dropout=1., n_layers=3):
    #     with tf.variable_scope(scope):
    #         # get variables
    #         input_size = input_enc.get_shape().as_list()[-1]
    #         weight = tf.get_variable('weight', [n_hidden, output_shape],
    #                                  initializer=tf.contrib.layers.xavier_initializer(),
    #                                  regularizer=tf.contrib.layers.l2_regularizer(self.weight_decay))
    ...
    #         rnn_cell = rnn.BasicLSTMCell(n_hidden, reuse=tf.AUTO_REUSE)
    #
    #     # reshape to [1, input_size]
    #     x = tf.reshape(input_enc, [-1, input_size])
    #     # Generate a sequence of inputs (eg. [had] [a] [general] -> [20] [6] [33])
    #     x = tf.split(x, input_size, 1)
    #
    #     # generate prediction
    #     outputs, states = rnn.static_rnn(rnn_cell, x, dtype=tf.float32)
    #
    #     # there are input_size outputs but we only want the last output
    #     out = tf.matmul(outputs[-1], weight) + bias
    #
    #     return out
