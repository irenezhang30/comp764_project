...
class GymEnvironment(object):
    ...
    def act(self, action):
        ...
        if self.args.lang_coeff > 0.0 and len(self.obs) >= 5:
    ...
    def compute_language_reward(self):
        if self.n_steps < 2:
            ...
            self.state_vector = self.get_state_vector(self.obs)
            with self.lang_net_graph.as_default():
                logits = self.lang_network.predict([self.action_vector], [self.lang], [self.state_vector])[0]
    ...
    def get_state_vector(self, obs):
        if len(obs) == 5:
            ...
            for i in reversed(range(5)):
                ...
                ob = ob.astype(np.float).reshape((64, 64, 3))
                latent = self.vae.encode(ob)
                self.state_vector.append(latent.tolist())
            self.state_vector = np.array(self.state_vector).reshape(-1).tolist()
            return self.state_vector
        else:
            raise ValueError("fix the error!")
